        ...
        if root and hum then
            ...
            if bestPrompt then
                if bestDist > (bestPrompt.MaxActivationDistance or 10) then
                    ...
                    local t = tick()
                    while not done and tick() - t < 5 do
                        task.wait(0.1)
                    end
                    pcall(function()
                        conn:Disconnect()
...
local function probeRay(origin, dir)
    ...
    if result then
        return result.Distance, result.Instance, result.Position
    end
...
local function steer(rootPos, targetPos)
    ...
    for i = 1, PROBE_COUNT do
        ...
        if ld > bestDist then
            bestDist = ld
            bestDir = rotateY(toTarget, -angle)
        end
        if rd > bestDist then
            bestDist = rd
            bestDir = rotateY(toTarget, angle)
...
local function addESP(p)
    ...
    if State.ESPCache[p.Name] and State.ESPCache[p.Name].Parent then
        return
    end
...
local function addNameTag(p)
    ...
    if not head or head:FindFirstChild("CB_Tag") then
        return
    end
...
local function toggleAutofarm(on)
    ...
    if on then
        startLoop("Autofarm", autofarmLoop)
    end
...
local function togglePathfind(on)
    ...
    State.PathOn = on
    setUiToggle("Pathfind", on)
    if on then
        startLoop("Pathfind", raycastPathfindLoop)
...
UserInputService.InputBegan:Connect(function(input, gp)
    ...
    if key == Keybinds.Autofarm then
        toggleAutofarm(not State.AutofarmOn)
    end
    if key == Keybinds.AutoHeal then
        toggleAutoHeal(not State.AutoHealOn)
    end
    if key == Keybinds.Pathfind then
        togglePathfind(not State.PathOn)
    end
    if key == Keybinds.AutoPinch then
        toggleAutoPinch(not State.AutoPinchOn)
    end
...
UiToggles.ResourceAura = ResTab:Toggle({
    ...
    Callback = function(v)
        ...
        if v then
            startLoop("ResourceAura", resourceAuraLoop)
        end
...
UiToggles.AutoHeal = HealTab:Toggle({
...
PinchTab:Slider({
    ...
    Callback = function(v)
        State.PinchInterval = v
    end,
})
UiToggles.AutoPinch = PinchTab:Toggle({
...
PathTab:Slider({
    ...
    Callback = function(v)
        State.PathStopDist = v
    end,
})
UiToggles.Pathfind = PathTab:Toggle({
...
EspTab:Paragraph({
    Title = "ESP Info",
    Desc = "Player and mob highlights are tracked separately.\nSome executors may block Highlight rendering.",
})
...
UiToggles.MobESP = EspTab:Toggle({
    Title = "Mob ESP",
    Value = false,
    Callback = function(v)
        State.MobESPOn = v
        if v then
            startLoop("MobESP", mobEspLoop)
        else
            clearMobESP()
        end
...
UiToggles.Tags = EspTab:Toggle({
    ...
    Callback = function(v)
        State.TagsOn = v
        if v then
            startLoop("NameTags", nameTagLoop)
        else
            clearAllTags()
        end
...
SetTab:Button({
    ...
    Callback = function()
        notify(
            "Credits",
            "Cherry Blossom Hub\nUI: WindUI\nFixes: single loops, safe ESP, Packets wait",
            5
        )
    end,
})
